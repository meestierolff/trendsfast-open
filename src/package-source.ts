...
const EXACT_GITHUB_SOURCE =
  /^github:meestierolff\/trendsfast-open#[0-9a-f]{40}$/;
const EXACT_NPM_SOURCE = new RegExp(
  `^${PACKAGE_NAME.replace("-", "\\-")}@${PACKAGE_VERSION.replaceAll(".", "\\.")}$`,
);
const GITHUB_RESOLVED =
  /(?:github\.com[\/:]meestierolff\/trendsfast-open(?:\.git)?[#\/])([0-9a-f]{40})(?:$|\?)/i;

export function validatePackageSource(value: string): string {
  const source = value.trim();
  if (EXACT_GITHUB_SOURCE.test(source) || EXACT_NPM_SOURCE.test(source))
    return source;
  throw new TrendsFastError(
    "INVALID_PACKAGE_SOURCE",
    "The package source must be the exact release version or immutable TrendsFast GitHub SHA.",
  );
}

function githubSourceFromResolved(value: unknown): string | null {
  if (typeof value !== "string") return null;
  if (EXACT_GITHUB_SOURCE.test(value)) return value;
...
function sourceFromAncestorLock(start: string): string | null {
  let directory = start;
    ...
    if (existsSync(candidate)) {
      try {
        ...
        for (const [path, metadata] of Object.entries(parsed.packages ?? {})) {
          if (
            path.endsWith(`/node_modules/${PACKAGE_NAME}`) ||
...
export function resolvePackageSource(
  ...
  const inferred = sourceFromAncestorLock(
