...
import {
  ...
  type CredentialMode,
  type InstallState,
  type SupportedClient as ConfigClient,
} from "./config.js";
import {
  installClientConfigurations,
  uninstallClientConfigurations,
  validateInstalledClientConfigurations,
} from "./clients.js";
...
type Command = "install" | "doctor" | "demo" | "mcp" | "uninstall" | "version";
...
export interface CliDependencies {
  ...
  hiddenInput?: (prompt: string) => Promise<string>;
  selectClients?: () => Promise<string[]>;
  /** Synthetic-test seam; the published CLI never supplies this. */
  backend?: BridgeBackend;
  /** Synthetic-test seam for credential rollback race coverage. */
  afterCredentialWrite?: () => Promise<void>;
}
...
function briefIdBindsScan(briefId: string, scanId: string): boolean {
...
function parseArguments(argv: readonly string[]): ParsedArguments {
  if (argv.some((argument) => /^--api[-_]?key(?:=|$)/i.test(argument))) {
    throw new TrendsFastError(
      "SECRET_IN_ARGV",
      "API keys are never accepted in command-line arguments.",
    );
  }
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    if (
      ...
      throw new TrendsFastError(
        "INVALID_ARGUMENT",
        "Use explicit supported flags without positional values.",
      );
    }
    ...
    if (!VALUE_FLAGS.has(argument)) {
      throw new TrendsFastError(
        "INVALID_ARGUMENT",
        "An unsupported command flag was provided.",
      );
    }
  ...
  return { command: command as Command, flags, values };
}
...
function rejectFlags(
  ...
  for (const flag of [...parsed.flags, ...parsed.values.keys()]) {
    if (!allowedSet.has(flag)) {
      throw new TrendsFastError(
        "INVALID_ARGUMENT",
        `${flag} is not valid for ${parsed.command}.`,
      );
    }
  }
}
...
function unwrapResult(result: CallToolResult): JsonObject {
  if (!isObject(result.structuredContent)) {
    throw new TrendsFastError(
      "REMOTE_PROTOCOL_FAILURE",
      "The tool result was not a structured object.",
    );
  }
  ...
  if (result.isError !== (envelope.ok === false)) {
    throw new TrendsFastError(
      "REMOTE_PROTOCOL_FAILURE",
      "The tool error signal did not match its structured envelope.",
    );
  }
  if (envelope.ok === true && isObject(envelope.data)) return envelope.data;
  ...
  throw new TrendsFastError(
    "REMOTE_PROTOCOL_FAILURE",
    "The tool result envelope was malformed.",
  );
}

function selectedBriefIsVideo(brief: JsonObject): boolean {
  if (isObject(brief.content_play))
    return brief.content_play.format === "VIDEO";
  return (
    isObject(brief.recommended_asset) &&
    brief.recommended_asset.format === "VIDEO"
  );
}

function outputJson(stdout: NodeJS.WritableStream, value: unknown): void {
  stdout.write(`${JSON.stringify(value, null, 2)}\n`);
}

function selectedCredentialMode(state: InstallState | null): CredentialMode {
  if (state === null || state.clients.length === 0) return "environment";
  const modes = [
  ...
  if (modes.length !== 1 || modes[0] === undefined) {
    throw new TrendsFastError(
      "INVALID_CONFIG",
      "Installed client credential modes are inconsistent.",
    );
  }
  return modes[0];
}
...
async function hiddenInputDefault(
  prompt: string,
  stdin: NodeJS.ReadableStream,
  stderr: NodeJS.WritableStream,
): Promise<string> {
  ...
  if (!tty.isTTY || typeof tty.setRawMode !== "function") {
    throw new TrendsFastError(
      "CREDENTIAL_REQUIRED",
      "Set TRENDSFAST_API_KEY in the inherited environment for non-interactive installation.",
    );
  }
  stderr.write(prompt);
  tty.setRawMode(true);
  ...
  try {
      ...
      const onData = (chunk: Buffer | string): void => {
        ...
        for (const character of text) {
          if (character === "\u0003") {
            cleanup();
            reject(
              new TrendsFastError("CANCELLED", "Installation was cancelled."),
            );
            return;
          }
          ...
          else if (character >= " " && value.length < 2048) value += character;
        }
      };
      const cleanup = (): void => {
        tty.off("data", onData);
    ...
    if (tty.isRaw) tty.setRawMode(false);
  }
}
...
async function confirmDefault(
  ...
  if (!tty.isTTY) {
    throw new TrendsFastError(
      ...
      "Use --yes only after reviewing the billable-create warning.",
    );
  }
...
async function selectClientsDefault(
  stdin: NodeJS.ReadableStream,
  stderr: NodeJS.WritableStream,
): Promise<string[]> {
  const tty = stdin as NodeJS.ReadStream;
  if (!tty.isTTY) {
    throw new TrendsFastError(
      "CLIENT_REQUIRED",
      "Select clients non-interactively with --client generic, --client claude-code, or --client codex.",
    );
  }
  const reader = createInterface({
    input: stdin,
  ...
  try {
    const answer = await reader.question(
      "Clients to configure (comma-separated: generic, claude-code, codex): ",
    );
    return answer
      .split(",")
...
function toConfigClient(value: string): ConfigClient {
  if (value === "generic") return "generic";
  if (value === "claude-code") return "claude";
  if (value === "codex") return "codex";
  throw new TrendsFastError(
    "UNSUPPORTED_CLIENT",
    "Supported clients are generic, claude-code, and codex.",
  );
}

async function backendFromConfig(
  dependencies: CliDependencies,
): Promise<{ backend: BridgeBackend; state: InstallState | null }> {
  ...
  if (
    state.protocolVersion !== REMOTE_MCP_PROTOCOL_VERSION ||
    ...
    throw new TrendsFastError(
      "INVALID_CONFIG",
      "The local TrendsFast install identity is stale or incomplete.",
    );
  }
...
async function installCommand(
  ...
  if (requestedClients.length === 0)
    throw new TrendsFastError(
      "CLIENT_REQUIRED",
      "Select at least one exact client.",
    );
      ...
      : rawMode === "file"
        ...
        : (() => {
            throw new TrendsFastError(
              ...
              "Use --credential-mode env or file.",
            );
          })();
  const endpoint = configuredEndpoint(one(parsed, "--endpoint")).href;
  const packageSource =
    one(parsed, "--package-source") === undefined
      ? resolvePackageSource(env)
      : validatePackageSource(one(parsed, "--package-source")!);
  const paths = resolveConfigPaths(configPathOptions(dependencies, env));
  const packageRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..");
  const skillBytes = await readRegularFile(
    resolve(packageRoot, "skills", "trendsfast", "SKILL.md"),
  );
  if (skillBytes === null)
    throw new TrendsFastError(
      "PACKAGE_INVALID",
      "The packaged TrendsFast skill is missing.",
    );
  ...
  let credentialCreatedSha256: string | null = null;
  if (credentialMode === "environment" && !dryRun) {
    await readCredential({ mode: "environment", paths, env });
  }
  if (credentialMode === "file" && !dryRun) {
    const apiKey =
      env.TRENDSFAST_API_KEY ??
      (await (
        dependencies.hiddenInput ??
        ((prompt) => hiddenInputDefault(prompt, stdin, stderr))
      )("TrendsFast project API key (hidden): "));
    if (!parsed.flags.has("--yes")) {
      throw new TrendsFastError(
        "CREDENTIAL_CONSENT_REQUIRED",
        "Protected-file mode requires --yes after explicit consent to store the credential.",
      );
    }
    ...
    const existingCredential = await readRegularFile(
      paths.credentialFile,
      trustedRootForPath(paths, paths.credentialFile),
    );
    if (existingCredential !== null) {
      ...
      if (existingValue !== apiKey) {
        throw new TrendsFastError(
          "CREDENTIAL_CONFLICT",
          "A different protected credential already exists; remove it explicitly before replacement.",
        );
      }
    } else {
  ...
  outputJson(stdout, {
    ...
    endpoint,
    clients: state.clients.map((record) =>
      record.client === "claude" ? "claude-code" : record.client,
    ),
...
async function doctorCommand(
  ...
  if (
    backend.tools.map((tool) => tool.name).join("\0") !==
    REMOTE_MCP_TOOL_NAMES.join("\0")
  ) {
    throw new TrendsFastError(
      "REMOTE_DESCRIPTOR_DRIFT",
      "The verified bridge inventory changed.",
    );
  }
  ...
  if (parsed.flags.has("--json")) outputJson(stdout, result);
  else
    stdout.write(
      "TrendsFast doctor: PASS (discovery, 7 tools, context, latest, sources; read-only)\n",
    );
}
...
async function maybeHandoff(
  ...
  if (!selectedBriefIsVideo(brief)) return null;
  const briefId = brief.brief_id;
  if (typeof briefId !== "string") {
    throw new TrendsFastError(
      "REMOTE_PROTOCOL_FAILURE",
      "A video brief did not expose its immutable brief ID.",
    );
  }
  return unwrapResult(
    await backend.callTool("trendsfast_creative_handoff_get", {
      brief_id: briefId,
    }),
  );
}
...
async function readOnlyDemo(
  ...
  const context = unwrapResult(
    await backend.callTool("trendsfast_project_context_get"),
  );
  const latest = unwrapResult(
    await backend.callTool("trendsfast_brief_latest_get"),
  );
  ...
  const existingStatus =
    scanId === undefined
      ? null
      : unwrapResult(
          await backend.callTool("trendsfast_today_status_get", {
            scan_id: scanId,
          }),
        );
  ...
  const historicalBrief =
    briefId === undefined
      ? null
      : unwrapResult(
          await backend.callTool("trendsfast_brief_get", { brief_id: briefId }),
        );
...
async function createDemo(
  ...
  if (key === undefined || !UUID_V4.test(key)) {
    throw new TrendsFastError(
      ...
      "--idempotency-key must be an explicit UUID v4.",
    );
  }
  ...
  if (
    ...
    throw new TrendsFastError(
      "INVALID_OBJECTIVE",
      "--objective must be explicit and between 1 and 100 characters.",
    );
  }
  ...
  if (
    ...
    throw new TrendsFastError(
      "INVALID_CAPABILITY",
      "Provide one or more unique --capability values.",
    );
  }
  for (const capability of requestedCapabilities) {
    if (!CONTENT_CAPABILITIES.includes(capability as ContentCapability)) {
      throw new TrendsFastError(
        "INVALID_CAPABILITY",
        "A supplied content capability is outside the public allowlist.",
      );
    }
  }
  stderr.write(
    "Warning: --create starts one billable TrendsFast scan. It never approves, delivers, publishes, or schedules.\n",
  );
  if (!parsed.flags.has("--yes")) {
    const confirmed = await (
      dependencies.confirm ??
      ((prompt) => confirmDefault(prompt, stdin, stderr))
    )("Type CREATE to confirm one billable scan: ");
    if (!confirmed)
      throw new TrendsFastError(
        "CANCELLED",
        "The billable scan was not confirmed.",
      );
  }
  ...
  if (
    ...
    throw new TrendsFastError(
      "REMOTE_PROTOCOL_FAILURE",
      "The project context did not expose its exact identity.",
    );
  }
  ...
  const admission = unwrapResult(
    await backend.callTool("trendsfast_today_create", createArguments),
  );
  const scanId =
    typeof admission.scan_id === "string" ? admission.scan_id : null;
  if (scanId === null) {
    throw new TrendsFastError(
      "REMOTE_PROTOCOL_FAILURE",
      "The accepted create result did not include a scan ID.",
    );
  }
  const sleep =
    dependencies.sleep ??
  ...
  for (let polls = 0; polls < 120; polls += 1) {
    ...
    if (delay !== 30) {
      throw new TrendsFastError(
        "REMOTE_PROTOCOL_FAILURE",
        "A running scan returned invalid poll timing.",
      );
    }
  ...
  if (!["REVIEW_REQUIRED", "READY", "FAILED"].includes(String(status.status))) {
    throw new TrendsFastError(
      "REMOTE_TIMEOUT",
      "The scan did not reach a terminal review state within the bounded poll window.",
    );
  }
  if (status.status === "FAILED") {
    ...
    throw new TrendsFastError(
      "SCAN_FAILED",
      "The scan reached terminal FAILED; it was not retried.",
    );
  }
  const latest = unwrapResult(
    await backend.callTool("trendsfast_brief_latest_get"),
  ...
  if (briefId === null) {
    throw new TrendsFastError(
      "REMOTE_PROTOCOL_FAILURE",
      "The completed brief did not expose its immutable identity.",
    );
  }
  if (!briefIdBindsScan(briefId, scanId)) {
    throw new TrendsFastError(
      "REMOTE_PROTOCOL_FAILURE",
      "The completed brief identity did not bind to the accepted scan.",
    );
  }
  const brief = unwrapResult(
    await backend.callTool("trendsfast_brief_get", { brief_id: briefId }),
  );
  if (canonicalJson(brief) !== canonicalJson(latest)) {
    throw new TrendsFastError(
      "REMOTE_PROTOCOL_FAILURE",
      "The latest brief did not match its immutable brief read.",
    );
  }
  ...
  if (
    ...
    throw new TrendsFastError(
      "REMOTE_PROTOCOL_FAILURE",
      "The completed brief did not bind to the requested project, context, lifecycle, objective, and capabilities.",
    );
  }
  const sources = unwrapResult(
    await backend.callTool("trendsfast_sources_get"),
  );
...
async function demoCommand(
  ...
  if (!parsed.flags.has("--create")) {
    if (
      one(parsed, "--idempotency-key") !== undefined ||
      one(parsed, "--objective") !== undefined ||
      many(parsed, "--capability").length > 0 ||
      parsed.flags.has("--yes")
    ) {
      throw new TrendsFastError(
        "CREATE_FLAG_REQUIRED",
        "Create inputs require the explicit --create flag.",
      );
    }
  ...
  if (
    one(parsed, "--scan-id") !== undefined ||
    one(parsed, "--brief-id") !== undefined
  ) {
    throw new TrendsFastError(
      "INVALID_ARGUMENT",
      "Historical read flags cannot be combined with --create.",
    );
  }
...
async function mcpCommand(
  ...
  await runBridge({
    ...
    ...(dependencies.fetch === undefined ? {} : { fetch: dependencies.fetch }),
    onError: (message) =>
      (dependencies.stderr ?? process.stderr).write(`trendsfast: ${message}\n`),
  });
}
...
async function uninstallCommand(
  ...
  else await removeInstallState(paths, dryRun);
  ...
  const installedSkillDirectories =
    state?.skills.map((record) => dirname(record.path)) ?? [];
  const genericSelected = state?.skills.some(
    (record) => record.client === "generic",
  );
  for (const directory of [
    ...
    await removeEmptyDirectory(
      directory,
      dryRun,
      trustedRootForPath(paths, directory),
    );
  }
  outputJson(stdout, {
    command: "uninstall",
...
function normalizedDoctorError(error: unknown): TrendsFastError {
  if (error instanceof TrendsFastError) return error;
  return new TrendsFastError(
    "LOCAL_INSTALL_INVALID",
    "The local TrendsFast install could not be verified. Uninstall and reinstall it before retrying Doctor.",
    { cause: error },
  );
}

function outputDoctorFailure(
  stdout: NodeJS.WritableStream,
  error: TrendsFastError,
): void {
  outputJson(stdout, {
    command: "doctor",
    status: "FAIL",
