...
export interface VerifiedRemoteContract {
...
function containsForbiddenProjectCapability(value: unknown): boolean {
  ...
  return Object.entries(value).some(
    ([key, nested]) =>
      ["project", "project_id", "projectId"].includes(key) ||
...
async function readBoundedJson(
  ...
  if (Number.isFinite(declared) && declared > maximumBytes) {
    throw new TrendsFastError(
      ...
      "The MCP response exceeded the safe size limit.",
    );
  }
  if (response.body === null) {
    throw new TrendsFastError(
      "REMOTE_PROTOCOL_FAILURE",
      "The MCP response body was empty.",
    );
  }
  ...
  try {
    ...
    throw new TrendsFastError(
      "REMOTE_PROTOCOL_FAILURE",
      "The MCP response was not valid JSON.",
    );
  }
}

function validateToolDescriptors(
  value: unknown,
  expectedDescriptorSha256: Readonly<Record<RemoteMcpToolName, string>>,
): Tool[] {
  if (!Array.isArray(value) || value.length !== REMOTE_MCP_TOOL_NAMES.length) {
    throw new TrendsFastError(
      ...
      "The Remote MCP tool count changed.",
    );
  }
  const tools: Tool[] = [];
  for (const [index, expectedName] of REMOTE_MCP_TOOL_NAMES.entries()) {
    ...
    if (!isObject(candidate) || candidate.name !== expectedName) {
      throw new TrendsFastError(
        "REMOTE_DESCRIPTOR_DRIFT",
        "The Remote MCP tool order changed.",
      );
    }
    if (!isObject(candidate.inputSchema) || !isObject(candidate.outputSchema)) {
      throw new TrendsFastError(
        "REMOTE_DESCRIPTOR_DRIFT",
        "A Remote MCP tool schema is missing.",
      );
    }
    if (!hasContractMarker(candidate.outputSchema)) {
      throw new TrendsFastError(
        "REMOTE_CONTRACT_DRIFT",
        "A Remote MCP tool lost its contract marker.",
      );
    }
    ...
    if (descriptorHash !== expectedDescriptorSha256[expectedName]) {
      throw new TrendsFastError(
        "REMOTE_DESCRIPTOR_DRIFT",
        "A Remote MCP tool descriptor changed.",
      );
    }
    tools.push(candidate as unknown as Tool);
  }
  return tools;
}
...
function validateApiKey(value: string): string {
  if (
    ...
    throw new TrendsFastError(
      "INVALID_CREDENTIAL",
      "The project-scoped API key is malformed.",
    );
  }
  return value;
}
...
export class RemoteMcpClient {
  ...
  private readonly expectedDescriptorSha256: Readonly<
  ...
  private async request(
    ...
    if (Buffer.byteLength(serializedBody) > MAX_REMOTE_REQUEST_BYTES) {
      throw new TrendsFastError(
        "INVALID_TOOL_INPUT",
        "The MCP request exceeded the safe size limit.",
      );
    }
    ...
    if (method === "tools/call") {
      ...
      if (name === "trendsfast_today_create") {
        ...
        if (typeof idempotencyKey !== "string") {
          throw new TrendsFastError(
            "INVALID_TOOL_INPUT",
            "The create idempotency key is missing.",
          );
        }
  ...
  async callTool(
    ...
    if (!REMOTE_MCP_TOOL_NAMES.includes(name)) {
      throw new TrendsFastError(
        "UNKNOWN_TOOL",
        "The requested tool is outside the exact Remote MCP V1 inventory.",
      );
    }
  ...
  getVerifiedTools(): readonly Tool[] {
    if (this.verifiedTools === null) {
      throw new TrendsFastError(
        "REMOTE_NOT_VERIFIED",
        "The Remote MCP contract has not been verified.",
      );
    }
